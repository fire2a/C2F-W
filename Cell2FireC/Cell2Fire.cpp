	...
	*/
	...
	if (this->args.WeatherOpt.compare("distribution") == 0)
	{
		...
		while (!selectWeather)
		{
			...
			{ // rd_number cannot be 1, but can be 0. So if prob weather has prob=0 it does not have a chance of being selected, and if it is 1, it will always be selected
		...
		*/
		...
		if (aux_list.size() > 0 && aux_list[0] != -100)
		{
			...
			if (this->args.verbose)
			{
				...
				{
					std::cout << "  Fire reaches the center of the cell " << msg << "  Distance to cell (in meters) was 100.0"
							  << " " << std::endl;
				}
			}
		}
...
void Cell2Fire::GetMessages(std::unordered_map<int, std::vector<int>> sendMessageList)
{
	...
	{
		if (this->args.verbose)
			std::cout << "Messages have been sent, next step. Current period: " << this->fire_period[this->year - 1] << std::endl;
		this->repeatFire = false;
	}
