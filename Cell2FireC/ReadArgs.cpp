...
void parseArgs(int argc, char *argv[], arguments *args_ptr) {
  ...
  char *input_folder =
      getCmdOption(argv, argv + argc, "--input-instance-folder");
  if (input_folder) {
    printf("InFolder: %s \n", input_folder);
  } else
    input_folder = &empty;
  ...
  if (simulator_option) {
    ...
    if (s != "S" && s != "K" && s != "C") {
      printf("%s Simulator Option not recognized or not developed, using S&B "
             "as default!!! \n",
             simulator_option);
  ...
  char *weather_period_len =
      getCmdOption(argv, argv + argc, "--Weather-Period-Length");
  ...
  //--Fire-Period-Length
  char *input_PeriodLen =
      getCmdOption(argv, argv + argc, "--Fire-Period-Length");
  if (input_PeriodLen) {
  ...
  //--CROSAct-Threshold
  char *CROSAct_Threshold =
      getCmdOption(argv, argv + argc, "--CROSAct-Threshold");
  ...
  //--max-fire-periods
  char *max_fire_periods =
      getCmdOption(argv, argv + argc, "--max-fire-periods");
  if (max_fire_periods) {
  ...
  if (!args_ptr->InFolder.empty() &&
      *args_ptr->InFolder.rbegin() != separator()) {
  ...
  if (!args_ptr->OutFolder.empty() &&
      *args_ptr->OutFolder.rbegin() != separator()) {
  ...
  if ((dir = opendir(directory_path.c_str())) != NULL) {
    while ((ent = readdir(dir)) != NULL) {
      if (ent->d_type == DT_REG) {
        ...
        if (filename.substr(0, 7) == "Weather" &&
