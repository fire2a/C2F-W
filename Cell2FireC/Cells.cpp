...
float Cells::slope_effect(float elev_i, float elev_j, double cellsize)
{
...
std::vector<int> Cells::manageFire(int period, std::unordered_set<int> &AvailSet,
								   ...
								   bool &activeCrown, double randomROS, double perimeterCells, std::vector<int> &crownState, std::vector<float> &crownFraction, std::vector<float> &surfFraction, std::vector<float> &Intensities, std::vector<float> &RateOfSpreads, std::vector<float> &FlameLengths)
{
...
std::vector<int> Cells::manageFireBBO(int period, std::unordered_set<int> &AvailSet,
									  inputs *df_ptr, fuel_coefs *coef,
									  std::vector<std::vector<int>> &coordCells, std::unordered_map<int, Cells> &Cells_Obj,
									  arguments *args, weatherDF *wdf_ptr, std::vector<double> *FSCell, std::vector<float> *crownMetrics,
									  bool &activeCrown, double randomROS, double perimeterCells, std::vector<float> &EllipseFactors, std::vector<int> &crownState, std::vector<float> &crownFraction, std::vector<float> &surfFraction, std::vector<float> &Intensities, std::vector<float> &RateOfSpreads, std::vector<float> &FlameLengths)
{
	// Special flag for repetition (False = -99 for the record)
	int repeat = -99;
...
bool Cells::get_burned(int period, int season, int NMsg, inputs df[], fuel_coefs *coef, arguments *args, weatherDF *wdf_ptr, bool &activeCrown, double perimeterCells)
{
	if (args->verbose)
	{
...
bool Cells::ignition(int period, int year, std::vector<int> &ignitionPoints, inputs *df_ptr, // WORKING CHECK OK
					 fuel_coefs *coef, arguments *args, weatherDF *wdf_ptr, bool &activeCrown, double perimeterCells)
{
...
/*
	...
	period       int
*/
void Cells::harvested(int id, int period)
{ // WORKING CHECK OK
	// TODO: unused param
	this->status = 3;
