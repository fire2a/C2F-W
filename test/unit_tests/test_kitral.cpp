...
TEST_CASE_METHOD(NativeFuelFixture, "Test fire type changes with intensity", "[fire_type]")
{
    int FMC = 100;
    test_data->cbh = 7.5;
    test_outs->sfi = 3200;
    REQUIRE(fire_type(test_data, test_outs, FMC) == false);
    test_outs->sfi = 4030;
    REQUIRE(fire_type(test_data, test_outs, FMC) == true);
    test_outs->sfi = 40420;
    REQUIRE(fire_type(test_data, test_outs, FMC) == true);
}


TEST_CASE_METHOD(NativeFuelFixture, "Test fire type changes with CBH", "[fire_type]")
{
    int FMC = 100;
    test_data->cbh = 7.5;
    test_outs->sfi = 3200;
...
public:
    PlantationFuelFixture()
    {
        ...
        strcpy(test_data->fueltype, "PL03");
        test_data->nftype = 21;
        test_data->ws = 10;
        test_data->waz = 45;
        test_data->tmp = 27;
        ...
        test_data->cbh = 11.05;
        test_data->cbd = 0.218;
        test_data->tree_height = 33;

        // Initialize fuel_coefs
        test_coefs->nftype = 21;
        test_coefs->fmc = 0.002481;
        test_coefs->cbh = 11.05;
        test_coefs->fl = 3.333;
        test_coefs->h = 20376;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Rate of spread changes with slope", "[rate_of_spread_k]")
{
    test_outs->se = 1;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(4.020, 0.001));
    test_outs->se = 0.5;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(3.708, 0.001));
    test_outs->se = 1.5;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(4.332, 0.001));
}

TEST_CASE_METHOD(PlantationFuelFixture, "PL Rate of spread changes with wind", "[rate_of_spread_k]")
{
    test_outs->se = 1.2;
    test_data->ws = 10;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(4.145, 0.001));
    test_data->ws = 50;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(8.426, 0.001));
    test_data->ws = 100;
    rate_of_spread_k(test_data, test_coefs, test_outs);
    REQUIRE_THAT(test_outs->rss, WithinAbs(9.245, 0.001));
}

//TODO: test reaction to temperature, humidity

TEST_CASE_METHOD(PlantationFuelFixture, "PL Test length to breadth", "[l_to_b]")
{
    REQUIRE_THAT(l_to_b(10, test_coefs), WithinAbs(1.058, 0.001));
    REQUIRE_THAT(l_to_b(100, test_coefs), WithinAbs(17.225, 0.001));
    //REQUIRE_THROWS(l_to_b(-1, test_coefs));
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test backfire ros", "[backfire_ros_k]")
{
    auto sec = new snd_outs();
    sec->lb = 1.058;
    test_outs->rss = 8;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test flankfire ros", "[flankfire_ros_k]")
{
    REQUIRE_THAT(flankfire_ros_k(8, 4, 1.058), WithinAbs(5.671, 0.001));
    REQUIRE_THAT(flankfire_ros_k(15, 0.126, 17.225), WithinAbs(0.439, 0.001));
}

TEST_CASE_METHOD(PlantationFuelFixture, "PL Test byram intensity", "[byram_intensity]")
{
    test_outs->rss = 15;
    REQUIRE_THAT(byram_intensity(test_data, test_outs), WithinAbs(16978.310, 0.001));
    test_outs->rss = 5.6;
    REQUIRE_THAT(byram_intensity(test_data, test_outs), WithinAbs(6338.569, 0.001));
}

TEST_CASE_METHOD(PlantationFuelFixture, "PL Test surface flame length", "[flame_length]")
{
    test_outs->sfi = 16873.87;
    REQUIRE_THAT(flame_length(test_data, test_outs), WithinAbs(6.82, 0.001));
    test_outs->sfi = 6299.58;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL PL Test flame angle", "[angleFL]")
{
    test_data->ws = 10;
    test_outs->fl = 6.82;
    REQUIRE_THAT(angleFL(test_data, test_outs), WithinAbs(1.127, 0.001));
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test flame angle changes with wind", "[angleFL]")
{
    test_data->ws = 100;
    test_outs->fl = 6.82;
    REQUIRE_THAT(angleFL(test_data, test_outs), WithinAbs(0.207, 0.001));
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test flame height", "[flame_height]")
{
    test_data->ws = 10;
    test_outs->fl = 6.82;
    REQUIRE_THAT(flame_height(test_data, test_outs), WithinAbs(6.160, 0.001));
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test flame height changes with wind", "[flame_height]")
{
    test_data->ws = 100;
    test_outs->fl = 6.82;
    float fl_100 = flame_height(test_data, test_outs);
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Active rate of spread changes with slope", "[active_rate_of_spreadPL04]")
{
    test_data->ws = 20;
    test_data->tmp = 27;
    test_data->rh = 40;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Active rate of spread changes with wind", "[active_rate_of_spreadPL04]")
{
    test_outs->se = 1.3;
    test_data->tmp = 27;
    test_data->rh = 40;
...
TEST_CASE_METHOD(PlantationFuelFixture,
                 "PL Active rate of spread changes with temperature",
                 "[active_rate_of_spreadPL04]")
{
    test_data->ws = 20;
    test_data->rh = 40;
    test_outs->se = 1.2;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Active rate of spread changes with humidity", "[active_rate_of_spreadPL04]")
{
    test_data->ws = 20;
    test_data->tmp = 26;
    test_outs->se = 1.2;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test check for active fire", "[checkActive]")
{
    int FMC = 100; // Default value in ReadArgs.cpp
    REQUIRE(checkActive(test_data, test_outs, FMC)==false);
    //TODO: use real cbd and cbh values
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test crown fraction burn", "[crownfractionburn]")
{
    int FMC = 100; // Default value in ReadArgs.cpp
    test_outs->rss = 8;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.542, 0.001));
    test_outs->rss = 15;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.947, 0.001));
    test_outs->rss = 30;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.999, 0.001));
}

TEST_CASE_METHOD(PlantationFuelFixture, "PL Test crown fraction burn changes with CBD", "[crownfractionburn]")
{
    int FMC = 100; // Default value in ReadArgs.cpp
    test_outs->rss = 8;
    test_data->cbd = 0.15;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.359, 0.001));
    test_data->cbd = 0.3;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.760, 0.001));
    test_data->cbd = -9999;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.441, 0.001));
    test_data->cbd = 0;
    REQUIRE_THAT(crownfractionburn(test_data, test_outs, FMC), WithinAbs(0.441, 0.001));
}


TEST_CASE_METHOD(PlantationFuelFixture, "PL Test final ros changes with ros", "[final_rate_of_spreadPL04]")
{
    test_outs->rss = 4;
    test_outs->ros_active = 10.3;
    test_outs->cfb = 0.89;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test final ros changes with ros active", "[final_rate_of_spreadPL04]")
{
    test_outs->rss = 4;
    test_outs->ros_active = 10.3;
    test_outs->cfb = 0.89;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test final ros changes with cfb", "[final_rate_of_spreadPL04]")
{
    test_outs->rss = 4;
    test_outs->ros_active = 10.3;
    test_outs->cfb = 0.89;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test backfire ros in crown fire changes with lb", "[backfire_ros10_k]")
{
    auto sec = new snd_outs();
    auto hptr = new fire_struc();
    sec->lb = 1.058;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test backfire ros in crown fire changes with ros", "[backfire_ros10_k]")
{
    auto sec = new snd_outs();
    auto hptr = new fire_struc();
    sec->lb = 1.058;
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test fire type changes with intensity", "[fire_type]")
{
    int FMC = 100;
    test_outs->sfi = 3200;
    REQUIRE(fire_type(test_data, test_outs, FMC) == false);
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test fire type changes with CBH", "[fire_type]")
{
    int FMC = 100;
    test_outs->sfi = 3200;
    REQUIRE(fire_type(test_data, test_outs, FMC) == false);
...
TEST_CASE_METHOD(PlantationFuelFixture, "PL Test setup canopy constants", "[setup_crown_const]")
{
    test_data->cbh = -9999;
    test_data->cbd = 0.01;
    setup_crown_const((test_data));
    REQUIRE_THAT(test_data->cbh, WithinAbs(11.05, 0.001));
    REQUIRE_THAT(test_data->cbd, WithinAbs(0.01, 0.001));
    test_data->cbd = -9999;
    setup_crown_const(test_data);
    REQUIRE_THAT(test_data->cbd, WithinAbs(0.218, 0.001));
}