...
for format in asc tif; do
    for model in fbp kitral sb; do
        ...
        rm -rf $output_folder/*
        if [ "$model" == "fbp" ]; then
            additional_args="--AllowCROS"
            sim_code="C"
        elif [ "$model" == "sb" ]; then
            additional_args="--AllowCROS --scenario 1"
            sim_code="S"
        elif [ "$model" == "kitral" ]; then
            additional_args=""
            sim_code="K"
        fi
        Cell2Fire$1 --InFolder model/$model-$format --OutFolder $output_folder --TotalSims 113 --OutMessages --OutputGrids --OutRos --OutIntensity --Simulator ${sim_code} --seed 42 --IgnitionsLog $additional_args > test_results/$model-$format/log.txt
