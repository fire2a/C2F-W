...
Ellipse::Ellipse(std::vector<double> _x, std::vector<double> _y) {
  ...
  M = C1.matrix().inverse() *
      (S1.matrix() -
       S2.matrix() * S3.matrix().inverse() * S2.matrix().transpose());
  EigenSolver<MatrixXd> M2(M.matrix());

  // M*|a b c >=l|a b c >. Find eigenvalues and eigenvectors from this equation
  // [eqn. 28]
  ...
  cond = 4 * Evec.row(0).array() * Evec.row(2).array() -
         Evec.row(1).array().pow(2);
...
std::vector<double> Ellipse::get_parameters() {
  ...
  double denominator1 =
      (b * b - a * c) *
      ((c - a) * std::sqrt(1 + 4 * b * b / ((a - c) * (a - c))) - (c + a));
  double denominator2 =
      (b * b - a * c) *
      ((a - c) * std::sqrt(1 + 4 * b * b / ((a - c) * (a - c))) - (c + a));
  double width = std::sqrt(numerator / denominator1);
  double height = std::sqrt(numerator / denominator2);

  // angle of counterclockwise rotation of major-axis of ellipse to x-axis [eqn.
  // 23] from (**) or [eqn. 26] from (***).
  this->_a = width;
  this->_b = height;
