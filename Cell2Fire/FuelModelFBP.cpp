...
which BA @ PFC pointed out may 93.......bmw 3.0  jan /96   bmw
      ...
      4.0 jan/98
        ...
        4.1  - August 2004
          - final changes in the FBP note (new M-3/M-4 model) and a new grass CF
model -removed some verstigaes of older functions for input /output 4.2 - Oct
2004
...
caluclated and used for final ROS in the C-6 model. 4.3 - August 2005
        - changes as a result of further discussions with Prometheus team. These
were:
        - changes to include the alternate ISI calculation formula (53a footnote
2) in the calculation of WSE  (...added as a section to The FBP Note)
...
void calculate_fbp(inputs *data, fuel_coefs *pt, main_outs *at, snd_outs *sec,
...
void determine_destiny_metrics_fbp(inputs *data, fuel_coefs *pt,
...
float ISF_mixedwood(fuel_coefs *ptr, float isz, int pc, float sf) {
  ...
  for (i = 0; strncmp(ptr->fueltype, "D1", 2) != 0 && i < numfuels; ptr++, i++)
    ;
  rsf_d1 =
      sf * (mult * ptr->p1) * pow((1.0 - exp(-1.0 * ptr->p2 * isz)), ptr->p3);
...
float fire_intensity(float fc, float ros) { return (300.0 * fc * ros); }
...
int fire_type(float csi, float sfi) { return (sfi > csi ? 1 : 0); }
...
float flank_spread_distance(inputs *inp, fire_struc *ptr, snd_outs *sec,
                            float hrost, float brost, float hd, float bd,
                            float lb, float a) {
...
float flank_fire_behaviour(inputs *inp, fuel_coefs *ptr, main_outs *at,
  ...
  if (firetype != 1 || at->covertype == 'n') {
    ...
    f->cfb = 0.0;
    f->fd = 'S';
    /*   f->ros=f->rss;  removed...v4.5   should not have been here ros set in
     * flankfire_ros()  */
  }
  return (fi);
}
