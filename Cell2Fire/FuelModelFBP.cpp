...
#include <string.h>
  ...
  void calculate_fbp(inputs *data,fuel_coefs *pt,main_outs *at,
...
/*   fuel type 0 */
   ...
   ptr->p1=110.0; ptr->p2=0.0282; ptr->p3=1.5;
...
/*   fuel type 2 */
   ...
   ptr->p1=120.0; ptr->p2=0.0572; ptr->p3=1.4;
   ptr->q=0.80; ptr->bui0=50; ptr->cbh=6; ptr->cfl=0.80;
/*   fuel type 3 */
   ptr++;
   strncpy(ptr->fueltype,"M4 ",3);
   ptr->p1=100.0; ptr->p2=0.0404; ptr->p3=1.48;
   ptr->q=0.80; ptr->bui0=50; ptr->cbh=6; ptr->cfl=0.80;
/*   fuel type 4 */
   ptr++;
   strncpy(ptr->fueltype,"C1 ",3);
   ptr->p1=90.0; ptr->p2=0.0649; ptr->p3=4.5;
...
/*  fuel type 17 */
 ...
 /* if all else has fail its a conifer   */
  ...
  {
    *mu=1.0;
    if(bui>=80) return( ptr->p1*pow( (1.0-exp(-1.0*ptr->p2*isi) ),ptr->p3) );
    else return (0.0);
  }
  ...
  /* ISI is ISZ really */
  {
    ...
    if(strncmp(ptr->fueltype,"M1",2)==0 || strncmp(ptr->fueltype,"M2",2)==0)
     ...
     if(rsf>0.0)check=1.0-pow((rsf/(mu*ptr->p1)),(1.0/ptr->p3) );
     else check=1.0;
 ...
 float ISF_mixedwood(fuel_coefs *ptr, float isz, int pc, float sf)
  {
    ...
    if(rsf_c2>0.0)check=1.0-pow((rsf_c2/(ptr->p1)),(1.0/ptr->p3) );
    else check=1.0;
    if(check<slopelimit_isi)check=slopelimit_isi;
    ...
    if(rsf_d1>0.0)check=1.0-pow((rsf_d1/(mult*ptr->p1)),(1.0/ptr->p3) );
    else check=1.0;
    if(check<slopelimit_isi)check=slopelimit_isi;
 ...
 float ISF_deadfir(fuel_coefs *ptr, float isz, int pdf, float sf)
  {
    ...
    if(rsf_max>0.0)check=1.0-pow((rsf_max/(ptr->p1)),(1.0/ptr->p3) );
    else check=1.0;
    if(check<slopelimit_isi)check=slopelimit_isi;
