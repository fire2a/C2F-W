...
char version[50] = "Last modified June 2015,  by BMW ", ver[17] = "Version 5.0001  ";
    ...
    inputs* data, fuel_coefs* pt, main_outs* at, snd_outs* sec, fire_struc* hptr, fire_struc* fptr, fire_struc* bptr)
{
    ...
    if (data->pattern == 1 && data->time > 0)
    {
        ...
        fptr->dist
            = flank_spread_distance(data, fptr, sec, hptr->rost, bptr->rost, hptr->dist, bptr->dist, sec->lb, accn);
        at->fl = hptr->dist;
...
float
flank_fire_behaviour(inputs* inp, fuel_coefs* ptr, main_outs* at, fire_struc* f)
{
    float sfi, fi;
    int firetype = 0;
    sfi = fire_intensity(at->sfc, f->rss);
