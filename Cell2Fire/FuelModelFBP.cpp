...
(1992-2005) It is provide free of charge to anyone who wishes to incorporate it
within their forest fire management applications, however users should note in
their application that the FBP calucaltions come from the Canadianf Forest
Services Fire Behaviour Prediction System. The Canadian Forest Service has gone
...
value in O1b was wrong.   KA at nofc (mar5,92) ...bmw 1.01a  - no error just
added upper and lowercase fueltype entry ...bmw 1.02   - l_to_b problem with
inequalitiy...minor prob fixed 1.5  - modifications to stop small(improbable)
numbers from blowing up the slope calculation stuff this caused a slope problem
which BA @ PFC pointed out may 93.......bmw 3.0  jan /96   bmw
        -  c6  - constant sfc
      3.01  apr/97
      ...
      4.0 jan/98
        ...
        4.1  - August 2004
          - final changes in the FBP note (new M-3/M-4 model) and a new grass
CF model -removed some verstigaes of older functions for input /output 4.2 -
Oct 2004
        - changes as a result of discussion with Prometheus team
         -  SFC in grass ...4.1 did not include changes to grass consumption
though spread at values of CUR<50% are now possible (see point 3.3 above)
        - at  very short time periods using acceleration a CFB(t) should be
caluclated and used for final ROS in the C-6 model. 4.3 - August 2005
        - changes as a result of further discussions with Prometheus team.
These were:
        - changes to include the alternate ISI calculation formula (53a
footnote 2) in the calculation of WSE  (...added as a section to The FBP Note)
        ...
        - NOTE that this (D-2) is not an official part of the FBP System
however. 4.4  August 2007
         - crown consump in m3/m4. It was calcuating based full conifer
content. but should be modifed by PDF
         - change to match changes in FBP Note section 3.3 (eqn 66c)
        4.5 Nov 2007
          - perimeter calc wasn't based on LB(t).
          - Flankfire final ROS (FROS) could be set to FRSS if flank fire did
not involve crowning
          - change to function headers in perimeter() and
flank_spread_distance() to accomodate LB(t) assignment
          - LB(t) is now added to the secondary inputs structure and kept
                it is asdsigned in  flank_spread_distance().
        4.6 jan 2009
          - uping the slope limit to 70%...and so the default after that
is 10.0 5.0  Oct 2014
...
void
calculate_fbp (inputs *data, fuel_coefs *pt, main_outs *at, snd_outs *sec,
               fire_struc *hptr, fire_struc *fptr, fire_struc *bptr)
{
  int firetype = 0;
  float accn;
  fuel_coefs **ptr = &pt;
  ...
  if (data->pattern == 1 && data->time > 0)
    {
      ...
      fptr->dist
          = flank_spread_distance (data, fptr, sec, hptr->rost, bptr->rost,
  ...
  if (data->pattern == 1 && data->time > 0)
    sec->perm = perimeter (hptr, bptr, sec, sec->lbt);
  else
    sec->perm = perimeter (hptr, bptr, sec, sec->lb);
  at->c = bptr->ros;
}

void
determine_destiny_metrics_fbp (inputs *data, fuel_coefs *pt,
                               main_outs *metrics, fire_struc *metrics2)
{
  int firetype = 0;
  fuel_coefs **ptr = &pt;
  setup_const (*ptr);
      ...
      if (metrics->covertype == 'c')
        {
          ...
          metrics->rso
              = critical_ros (data->fueltype, metrics->sfc, metrics->csi);
...
ros_calc (inputs *inp, fuel_coefs *ptr, float isi, float *mult)
{
  ...
  if (strncmp (inp->fueltype, "M1", 2) == 0
      || strncmp (inp->fueltype, "M2", 2) == 0)
    return (mixed_wood (ptr, isi, mult, inp->pc));
  if (strncmp (inp->fueltype, "M3", 2) == 0
      || strncmp (inp->fueltype, "M4", 2) == 0)
...
grass (fuel_coefs *ptr, float cur, float isi, float *mult)
{
  float mu, ros;
  if ((float)(cur) >= 58.8)
    mu = 0.176 + 0.02 * ((float)(cur)-58.8);
  else
    mu = 0.005 * (exp (0.061 * (float)(cur)) - 1.0);
  ros = mu * (ptr->p1 * pow ((1.0 - exp (-1.0 * ptr->p2 * isi)), ptr->p3));
  if (mu < 0.001)
...
mixed_wood (fuel_coefs *ptr, float isi, float *mu, int pc)
{
  ...
  *mu = pc / 100.0;
  ...
  if (strncmp (ptr->fueltype, "M2", 2) == 0)
    ...
    mult = 1.0;
  for (i = 0; strncmp (ptr->fueltype, "D1", 2) != 0 && i < numfuels;
       ptr++, i++)
    ;
  if (i >= numfuels)
    {
...
dead_fir (fuel_coefs *ptr, int pdf, float isi, float *mu)
{
  ...
  ros = (float)(pdf) / 100.0 * rosm3or4_max
        + (100.0 - (float)(pdf)) / 100.0 * greenness * ros_d1;
...
/* ISI is ISZ really */
{
  double isf, rsf, wse, ps, rsz, wsx, wsy, wsex, wsey, wsvx, wsvy, wrad, srad,
      wsv, raz, check, wse2, wse1;
  float mu = 0.0;
  ps = inp->ps * 1.0;
  ...
  if (strncmp (ptr->fueltype, "M1", 2) == 0
      || strncmp (ptr->fueltype, "M2", 2) == 0)
    isf = ISF_mixedwood (ptr, isi, inp->pc, at->sf);
  else if (strncmp (ptr->fueltype, "M3", 2) == 0
           || strncmp (ptr->fueltype, "M4", 2) == 0)
  ...
  else
    {
      ...
      if (rsf > 0.0)
        check = 1.0 - pow ((rsf / (mu * ptr->p1)), (1.0 / ptr->p3));
      else
        check = 1.0;
      if (check < slopelimit_isi)
        check = slopelimit_isi;

      isf = (1.0 / (-1.0 * ptr->p2)) * log (check);
    }
  if (isf == 0.0)
  ...
  if (wse1 <= 40.0)
    ...
    {
      if (isf > (0.999 * 2.496 * at->ff))
        isf = 0.999 * 2.496 * at->ff;
...
ISF_mixedwood (fuel_coefs *ptr, float isz, int pc, float sf)
{
  ...
  rsf_c2 = sf * ptr->p1 * pow ((1.0 - exp (-1.0 * ptr->p2 * isz)), ptr->p3);
  if (rsf_c2 > 0.0)
    check = 1.0 - pow ((rsf_c2 / (ptr->p1)), (1.0 / ptr->p3));
  else
    check = 1.0;
  if (check < slopelimit_isi)
    check = slopelimit_isi;
  ...
  for (i = 0; strncmp (ptr->fueltype, "D1", 2) != 0 && i < numfuels;
       ptr++, i++)
    ;
  rsf_d1 = sf * (mult * ptr->p1)
           * pow ((1.0 - exp (-1.0 * ptr->p2 * isz)), ptr->p3);

  if (rsf_d1 > 0.0)
    check = 1.0 - pow ((rsf_d1 / (mult * ptr->p1)), (1.0 / ptr->p3));
  else
    check = 1.0;
  if (check < slopelimit_isi)
    check = slopelimit_isi;
  isf_d1 = (1.0 / (-1.0 * ptr->p2)) * log (check);

  return (((float)(pc) / 100.0) * isf_c2
          + (100 - (float)(pc)) / 100.0 * isf_d1);
}

float
ISF_deadfir (fuel_coefs *ptr, float isz, int pdf, float sf)
{
  float check, mult, rsf_d1, rsf_max, isf_d1, isf_max;
  int i;

  rsf_max = sf * ptr->p1 * pow ((1.0 - exp (-1.0 * ptr->p2 * isz)), ptr->p3);
  if (rsf_max > 0.0)
    check = 1.0 - pow ((rsf_max / (ptr->p1)), (1.0 / ptr->p3));
  else
    check = 1.0;
  if (check < slopelimit_isi)
    check = slopelimit_isi;
  ...
  return (((float)(pdf) / 100.0) * isf_max
          + (100.0 - (float)(pdf)) / 100.0 * isf_d1);
}
  ...
  if (inp->jd_min <= 0)
    {
      if (inp->elev < 0)
      ...
      else
        {
          ...
          at->jd_min
              = (int)(0.5 + 142.1 * inp->lat / latn + (0.0172 * inp->elev));
        }
    }
...
crit_surf_intensity (fuel_coefs *ptr, float fmc)
{
  if (ptr->cbh > 0)
    ...
    {
      return (999999999); // if cbh is equal to 0, critical intensity is set to
                          // big number
    }
}
...
int
fire_type (float csi, float sfi)
{
  return (sfi > csi ? 1 : 0);
}

char
fire_description (float cfb)
{
  if (cfb < 0.1)
    return ('S');
  if (cfb < 0.9 && cfb >= 0.1)
...
void
set_all (fire_struc *ptr, int time)
{
  ptr->time = 0;
  ptr->rost = ptr->ros;
...
{
  float mult, p;
  mult = 3.1415926 * (1.0 + 1.0 / lb)
         * (1.0 + pow (((lb - 1.0) / (2.0 * (lb + 1.0))), 2.0));
...
{
  int i;
  char canopy = 'c', open_list[10][3] = { "O1", "C1", "S1", "S2", "S3",
                                          "o1", "c1", "s1", "s2", "s3" };
...
float
flank_spread_distance (inputs *inp, fire_struc *ptr, snd_outs *sec,
                       float hrost, float brost, float hd, float bd, float lb,
                       float a)
{
...
int
time_to_crown (float ros, float rso, float a)
{
  float ratio;
  if (ros > 0)
    ratio = rso / ros;
  ...
  if (ratio > 0.9 && ratio <= 1.0)
    ratio = 0.9;
  if (ratio < 1.0)
    return (int)(log (1.0 - ratio) / -a);
  else
    return (99);
}

float
fire_behaviour (inputs *inp, fuel_coefs *ptr, main_outs *at, fire_struc *f)
{
  float sfi, fi;
  int firetype = 0;
...
float
flank_fire_behaviour (inputs *inp, fuel_coefs *ptr, main_outs *at,
                      fire_struc *f)
{
  float sfi, fi;
  int firetype = 0;
  ...
  if (firetype != 1 || at->covertype == 'n')
    {
      ...
      f->cfb = 0.0;
      f->fd = 'S';
      /*   f->ros=f->rss;  removed...v4.5   should not have been here ros set
       * in flankfire_ros()  */
    }
  return (fi);
}
