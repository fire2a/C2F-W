...
typedef struct {
  ...
  float ws, saz, cur, ps, cbh, ccf, cbd, elev, tmp, rh, lat, lon, ffmc, bui,
      gfl, height; // David: some variables only used on C2FK and not on C2FSB
                   // and viceversa
...
typedef struct {
  float hffmc, sfc, csi, fl, fh, a, b, c, rss, angle, ros_active, cfb, se, rso,
      fmc, sfi, isi, be, sf, raz, wsv, ff, crown_intensity, crown_flame_length;
  char covertype;
  int crown, jd_min, jd;
} main_outs;
...
public:
  ...
  std::string fType2;
  std::vector<int>
      coord; // maybe change this into a tuple or class	CP: 2-tuple (int)
  ...
  std::unordered_map<int, double>
      ROSAngleDir; // CP: dictionary {int: double|None}   Instead of None we can
                   // use a determined number like -9999 = None  TODO: maybe int
                   // : double
  ...
  Cells(int _id, double _area, std::vector<int> _coord, int _fType,
  ...
  void initializeFireFields(std::vector<std::vector<int>> &coordCells,
                            std::unordered_set<int> &availSet, int cols,
                            int rows); // TODO: need TYPE
  void ros_distr_old(double thetafire, double forward, double flank,
                     double back);
  double rhoTheta(double theta, double a, double b);
  void ros_distr(double thetafire, double forward, double flank, double back,
                 double EFactor);
  void ros_distr_V2(double thetafire, double a, double b, double c,
                    double EFactor);

  std::vector<int> manageFire(
      int period, std::unordered_set<int> &AvailSet, inputs df[],
  ...
  bool get_burned(int period, int season, int NMsg, inputs df[],
