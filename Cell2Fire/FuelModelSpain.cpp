...
l_to_b (float ws)
{
  ...
  return pow ((0.936 * exp (alpha * factor * ws)
               + 0.461 * exp (beta * factor * ws) - 0.397),
              0.45);
}
...
void
calculate_s (inputs *data, fuel_coefs *ptr, arguments *args, main_outs *at,
...
void
determine_destiny_metrics_s (inputs *data, fuel_coefs *ptr, arguments *args,
                             main_outs *metrics)
{
  // Hack: Initialize coefficients
  initialize_coeff (args->scenario);
