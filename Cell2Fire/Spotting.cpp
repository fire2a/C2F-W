          ...
          std::unordered_map<std::string, double> spottingParams, bool verbose)
{
  ...
  // TODO
  std::unordered_map<int, std::unordered_map<int, int> > Angles
      = std::unordered_map<int, std::unordered_map<int, int> > ();
  std::unordered_map<int, std::unordered_map<int, int> > Distances
      = std::unordered_map<int, std::unordered_map<int, int> > ();
  std::unordered_map<int, std::unordered_map<int, double> > SpotProb
      = std::unordered_map<int, std::unordered_map<int, double> > ();

  if (spottingParams["SPOTANGLE"] * spottingParams["SPOT0PROB"]
          * spottingParams["SPOT10TIME"] * WSC
      == 0)
    {
      return vector<int> ();
    }
  ...
  if (verbose)
    {
      ...
      std::cout << "Spotting Wind Windows: [" << WA << ", " << WB << "]"
                << std::endl;
    }
      ...
      for (auto &c2 : AvailSet)
        {
          ...
          if (Angles[c1][c2] == -1)
            ...
            {
              SpotProb[c1][c2]
                  = beta * std::exp (-1 * Distances[c1][c2] * alpha);
              double WA_t = WA;
              double WB_t = WB;
              ...
              if (WA_t < 0 && WB_t >= 0 && WB_t < 360 && WTolerance < 180)
                {
                  if (!(Angles[c1][c2] <= WB_t
                        || Angles[c1][c2] >= WA_t + 360))
                    {
                      SpotProb[c1][c2] = 0;
                    }
                }
  ...
  // MODIFYING!!!!  got rid of  msg_list out of scope, got rid of c2 problem,
  // BUT CHECK!!!!!!!!!! COMPILER THROWS ERROR:

  /*
  In file included from SpottingFBP.h:4:0,
           from SpottingFBP.cpp:2:
CellsFBP.h:57:9: note: candidate: CellsFBP::CellsFBP(int, double,
std::vector<int>, double, int, std::__cxx11::string, double, double, int,
std::unordered_map<std::__cxx11::basic_string<char>, std::vector<int> >&,
std::vector<double>, int, bool) CellsFBP(int _id, double _area,
std::vector<int> _coord, double _age,
