...
void DataGrids(const std::string &filename, std::vector<float> &data,
               int nCells) {
  std::ifstream file(filename);
  if (!file.is_open()) {
...
void DataGridsTif(const std::string &filename, std::vector<float> &data,
                  int nCells) {
  /*
  Reads fuel data from a .tif
  Args:
  filename (std::string): Name of .tif file.
  Dictionary (std::unordered_map<std::string, std::string>&): Reference to fuels
  dictionary
...
GenerateDat(const std::vector<std::string> &GFuelType,
  ...
  std::vector<std::string> Columns = {
      "fueltype", "lat", "lon",     "elev",  "ws",  "waz",    "ps",
      "saz",      "cur", "cbd",     "cbh",   "ccf", "ftypeN", "fmc",
      "py",       "jd",  "jd_min",  "pc",    "pdf", "time",   "ffmc",
      "bui",      "gfl", "pattern", "height"};
  ...
  for (size_t i = 0; i < GFuelType.size(); ++i) {
    ...
    if (std::isnan(Height[i])) {
      ...
      rowData.emplace_back(
          std::make_unique<std::string>(std::to_string(Height[i])));
    }
...
void GenDataFile(const std::string &InFolder, const std::string &Simulator) {
  ...
  */
  ...
  std::vector<std::vector<std::unique_ptr<std::string>>> result =
      GenerateDat(GFuelType, GFuelTypeN, Elevation, PS, SAZ, Curing, CBD, CBH,
                  CCF, PY, FMC, Height, InFolder);
