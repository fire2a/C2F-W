      ...
      if (aux > 1)
        {
          ...
          else if (tokens[3].substr (0, 3) == "Non"
                   || tokens[3].substr (0, 3) == "NFn")
            {
              FBPDict[tokens[0]] = "NF";
            }
          else
          ...
          ColorsDict[tokens[0]] = std::make_tuple (
              std::stof (tokens[4]) / 255.0f, std::stof (tokens[5]) / 255.0f,
...
ForestGrid (const std::string &filename,
            const std::unordered_map<std::string, std::string> &Dictionary)
{
  ...
  if (!(iss >> key >> value) || key != "cellsize")
    {
      ...
      throw std::runtime_error ("Expected 'cellsize' on line 5 of "
                                + filename);
    }
  ...
  return std::make_tuple (gridcell3, gridcell4, grid.size (), tcols - 1,
                          cellsize);
}
...
DataGrids (const std::string &filename, std::vector<float> &data, int nCells)
{
  ...
  if (!(iss >> key >> value) || key != "cellsize")
    {
      std::cerr << "Error: Expected 'cellsize' on line 5 of " << filename
                << std::endl;
      return;
    }
...
std::tuple<std::vector<int>, std::vector<std::string>, int, int, float>
ForestGridTif (const std::string &filename,
               const std::unordered_map<std::string, std::string> &Dictionary)
{
  /*
  ...
  */
  ...
  if (n_bits == 64)
    ...
    {
      throw std::runtime_error ("Error: file type is not supported: '"
                                + filename + "'");
    }
      ...
      for (int j = 0; j < nXSize; j++)
        {
          ...
          if (pixelValue != pixelValue
              || Dictionary.find (token) == Dictionary.end ())
            {
...
void
DataGridsTif (const std::string &filename, std::vector<float> &data,
              int nCells)
{
  /*
  Reads fuel data from a .tif
             ...
             const std::vector<float> &FMC, const std::string &InFolder)
{
  ...
  std::vector<std::string> Columns = {
    "fueltype", "lat", "lon", "elev", "ws",     "waz", "ps",  "saz",
    "cur",      "cbd", "cbh", "ccf",  "ftypeN", "fmc", "py",  "jd",
    "jd_min",   "pc",  "pdf", "time", "ffmc",   "bui", "gfl", "pattern"
  };
  ...
  std::unordered_map<std::string, int> PDFD = {
    { "M3_5", 5 },     { "M3_10", 10 },   { "M3_15", 15 },   { "M3_20", 20 },
    { "M3_25", 25 },   { "M3_30", 30 },   { "M3_35", 35 },   { "M3_40", 40 },
    { "M3_45", 45 },   { "M3_50", 50 },   { "M3_55", 55 },   { "M3_60", 60 },
    { "M3_65", 65 },   { "M3_70", 70 },   { "M3_75", 75 },   { "M3_80", 80 },
    { "M3_85", 85 },   { "M3_90", 90 },   { "M3_95", 95 },   { "M4_5", 5 },
    { "M4_10", 10 },   { "M4_15", 15 },   { "M4_20", 20 },   { "M4_25", 25 },
    { "M4_30", 30 },   { "M4_35", 35 },   { "M4_40", 40 },   { "M4_45", 45 },
    { "M4_50", 50 },   { "M4_55", 55 },   { "M4_60", 60 },   { "M4_65", 65 },
    { "M4_70", 70 },   { "M4_75", 75 },   { "M4_80", 80 },   { "M4_85", 85 },
    { "M4_90", 90 },   { "M4_95", 95 },   { "M3M4_5", 5 },   { "M3M4_10", 10 },
    { "M3M4_15", 15 }, { "M3M4_20", 20 }, { "M3M4_25", 25 }, { "M3M4_30", 30 },
    { "M3M4_35", 35 }, { "M3M4_40", 40 }, { "M3M4_45", 45 }, { "M3M4_50", 50 },
    { "M3M4_55", 55 }, { "M3M4_60", 60 }, { "M3M4_65", 65 }, { "M3M4_70", 70 },
    { "M3M4_75", 75 }, { "M3M4_80", 80 }, { "M3M4_85", 85 }, { "M3M4_90", 90 },
    { "M3M4_95", 95 }
  };
  ...
  for (size_t i = 0; i < GFuelType.size (); ++i)
    {
      ...
      if (std::isnan (Elevation[i]))
      ...
      else
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (Elevation[i])));
        }
      ...
      if (std::isnan (PS[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (PS[i])));
        }
      ...
      if (std::isnan (SAZ[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (SAZ[i])));
        }

      // Handle special cases 8
      if (std::isnan (Curing[i])
          && (GFuelType[i] == "O1a" || GFuelType[i] == "O1b"))
        {
      ...
      if (std::isnan (CBD[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (CBD[i])));
        }
      ...
      if (std::isnan (CBH[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (CBH[i])));
        }
      ...
      if (std::isnan (CCF[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (CCF[i])));
        }
      ...
      if (std::isnan (static_cast<double> (GFuelTypeN[i])))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (GFuelTypeN[i])));
        }
      ...
      if (std::isnan (FMC[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (FMC[i])));
        }
      ...
      if (std::isnan (PY[i]))
        ...
        {
          rowData.emplace_back (
              std::make_unique<std::string> (std::to_string (PY[i])));
        }
      ...
      if (PCD.find (GFuelType[i]) != PCD.end ())
        {
          rowData.emplace_back (std::make_unique<std::string> (
              std::to_string (PCD[GFuelType[i]]))); // "pc"
        }
      else
        {
      ...
      if (PDFD.find (GFuelType[i]) != PDFD.end ())
        {
          rowData.emplace_back (std::make_unique<std::string> (
              std::to_string (PDFD[GFuelType[i]]))); // "pdf"
        }
      else
        {
          rowData.emplace_back (std::make_unique<std::string> (""));
      ...
      if (GFLD.find (GFuelType[i]) != GFLD.end ())
        {
          rowData.emplace_back (std::make_unique<std::string> (
              std::to_string (GFLD[GFuelType[i]]))); // "gfl"
        }
      else
        {
          rowData.emplace_back (std::make_unique<std::string> (""));
...
void
writeDataToFile (
    const std::vector<std::vector<std::unique_ptr<std::string> > > &dataGrids,
      ...
      for (const auto &rowData : dataGrids)
        {
          for (const auto &item : rowData)
            {
              dataFile << *item
                       << ","; // Dereference the unique_ptr before writing
            }
          dataFile << "\n";
        }
...
GenDataFile (const std::string &InFolder, const std::string &Simulator)
{
  ...
  if (Simulator == "K")
    ...
    {
      std::cerr << "Error: Simulator not recognized:" << Simulator
                << std::endl;
      exit (1);
    }
  ...
  if (!fileExists (lookupTable))
    {
      std::cerr << "Error: Lookup table '" << lookupTable << "' not found"
                << std::endl;
      return;
    }
  ...
  if (extension == ".tif")
    {
      std::tie (GFuelTypeN, GFuelType, FBPDicts, Cols, CellSide)
  ...
  */
  ...
  std::vector<std::string> filenames
      = { "elevation" + extension, "saz" + extension, "slope" + extension,
          "cur" + extension,       "cbd" + extension, "cbh" + extension,
          "ccf" + extension,       "py" + extension,  "fmc" + extension };
  ...
  std::vector<std::vector<std::unique_ptr<std::string> > > result
      = GenerateDat (GFuelType, GFuelTypeN, Elevation, PS, SAZ, Curing, CBD,
                     CBH, CCF, PY, FMC, InFolder);
