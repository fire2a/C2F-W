          ...
          if (header < 5)
            {
              ...
              int start = 0;
              int end = 0;
              while ((start = line.find_first_not_of (this->delimeter, end))
                     != std::string::npos)
                {
          ...
          else
            {
              ...
              boost::algorithm::split (vec, line,
                                       boost::is_any_of (this->delimeter));
              dataList.push_back (vec);
            }
        }
    }
  else if (this->fileName.substr (this->fileName.find_last_of (".") + 1)
           == "tif")
    {
      ...
      if (fabs (cellSizeX - cellSizeY) > epsilon)
        {
          throw std::runtime_error ("Error: Cells are not square in: '"
                                    + fileName + "'");
        }
      double *positions;
          ...
          if (TIFFReadScanline (fuelsDataset, buf, i) != 1)
            {
              _TIFFfree (buf);
              TIFFClose (fuelsDataset);
              throw std::runtime_error ("Read error on row "
                                        + std::to_string (i));
            }
          // For each column
          std::string token;
          for (int j = 0; j < nXSize; j++)
            {
              ...
              if (n_bits == 64)
                {
                  token
                      = std::to_string (static_cast<int> (((double *)buf)[j]));
                }
              else
                {
                  token = std::to_string (
                      static_cast<int> (((int32_t *)buf)[j]));
                }
 ...
 */
void
CSVReader::parseDF (inputs *df_ptr, std::vector<std::vector<std::string> > &DF,
                    arguments *args_ptr, int NCells)
{
  int i;
 ...
 */
void
CSVReader::parseNDF (std::vector<int> &NFTypes,
                     std::vector<std::vector<std::string> > &DF, int NCells)
{
  int i;
 ...
 */
void
CSVReader::parsePROB (std::vector<float> &probabilities,
                      std::vector<std::vector<std::string> > &DF, int NCells)
{
  int i;
 ...
 */
void
CSVReader::parseWeatherDF (weatherDF *wdf_ptr, arguments *args_ptr,
 ...
 */
void
CSVReader::parseIgnitionDF (std::vector<int> &ig,
                            std::vector<std::vector<std::string> > &DF,
                            int IgPeriods)
{
  // Integers
...
CSVReader::parseBBODF (std::unordered_map<int, std::vector<float> > &bbo,
                       std::vector<std::vector<std::string> > &DF, int NFTypes)
{
  // Integers
  int i, j, ftype;
  int ffactors = 4;
...
void
CSVReader::parseForestDF (forestDF *frt_ptr,
                          std::vector<std::vector<std::string> > &DF)
{
  // Ints
