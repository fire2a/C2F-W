 ...
 */
template <typename T> void CSVWriter::addDatainRow(T first, T last) {
  std::fstream file;
  // Open the file in truncate mode if first line else in Append Mode
  file.open(this->fileName,
            std::ios::out |
                (this->linesCount ? std::ios::app : std::ios::trunc));
...
void CSVWriter::asciiHeader(int rows, int cols, double xllcorner,
                            double yllcorner, int cellside) {
  std::fstream file;
  // Open the file in truncate mode if first line else in Append Mode
  file.open(this->fileName,
            std::ios::out |
  ...
  for (r = 0; r < rows; r++) {
    for (c = 0; c < cols; c++) {
      ...
      std::vector<int>::const_iterator first =
          statusCells.begin() + c + r * cols;
      std::vector<int>::const_iterator last =
          statusCells.begin() + c + r * cols + cols;
 ...
 *     Creates CSVDouble
 */
void CSVWriter::printCSVDouble(int rows, int cols,
  ...
  for (r = 0; r < rows; r++) {
    ...
    for (c = 0; c < cols; c++) {
      ...
      std::vector<double>::const_iterator first =
          network.begin() + c + r * cols;
      std::vector<double>::const_iterator last =
          network.begin() + c + r * cols + cols;
...
void CSVWriter::printCSVDouble_V2(int rows, int cols,
                                  std::vector<double> network) {
  bool outs = false;
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      ofs << (int)network[c + r * cols] << this->delimeter
          << (int)network[c + r * cols + 1] << this->delimeter
          << (int)network[c + r * cols + 2] << this->delimeter
          << network[c + r * cols + 3] << "\n";
      c += cols;
    }
...
void CSVWriter::printASCII(int rows, int cols, double xllcorner,
                           double yllcorner, int cellside,
...
void CSVWriter::printASCIIInt(int rows, int cols, double xllcorner,
                              double yllcorner, int cellside,
...
void CSVWriter::printCSV_V2(int rows, int cols, std::vector<int> statusCells) {
  ...
  for (r = 0; r < rows; r++) {
    for (c = 0; c < cols; c++) {
      ...
      for (i = 0; i < cols; i++) {
        toOut +=
            std::to_string(statusCells[c + r * cols + i]) + this->delimeter;
      }
      ofs << toOut << "\n";
      c += cols;
    }
