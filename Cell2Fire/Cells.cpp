      ...
      if (nb != -1)
        {
          ...
          this->distToCenter[nb]
              = std::sqrt (a * a + b * b) * this->_ctr2ctrdist;
        }
    }
}
...
adjacentCells (int cell, int nrows, int ncols)
{
  ...
  int southeast = cell + ncols > total_cells || cell % ncols == 0
                      ? -1
                      : cell + ncols + 1;
  int southwest = cell % ncols == 1 || cell + ncols > total_cells
                      ? -1
                      : cell + ncols - 1;
 ...
 */
void
Cells::ros_distr_old (double thetafire, double forward, double flank,
                      double back)
{ // WORKING CHECK OK
 ...
 */
void
Cells::ros_distr (double thetafire, double forward, double flank, double back,
                  double EFactor)
{ // WORKING CHECK OK
 ...
 */
void
Cells::ros_distr_V2 (double thetafire, double a, double b, double c,
                     double EFactor)
{
    ...
    std::vector<float> &FlameLengths)
{
  ...
  if (args->verbose)
    {
      ...
      std::cout << "fueltype: " << df_ptr[this->realId - 1].fueltype
                << std::endl;
      std::cout << "nfueltype: " << df_ptr[this->realId - 1].nftype
                << std::endl;
  ...
  cartesianAngle = 360 - (offset >= 360) * (cartesianAngle - 90)
                   - (offset < 360) * offset;
  ...
  if (args->verbose)
    {
      ...
      std::cout << "Rear ROS Value: " << backstruct.ros * args->BFactor
                << std::endl;
      std::cout << "Std Normal RV for Stochastic ROS CV: " << ROSRV
                << std::endl;
    }
          ...
          if (args->Simulator == "S")
            {
              // Slope effect
              float se
                  = slope_effect (df_ptr[this->realId - 1].elev,
          ...
          if (this->fireProgress[nb] >= this->distToCenter[nb])
            {
              ...
              if (args->Simulator == "K")
                {
                  determine_destiny_metrics_k (&df_ptr[int (nb) - 1], coef,
                                               args, &metrics);
                }
              else if (args->Simulator == "S")
                {
                  determine_destiny_metrics_s (&df_ptr[int (nb) - 1], coef,
                                               args, &metrics);
                }
              else if (args->Simulator == "C")
                {
                  determine_destiny_metrics_fbp (&df_ptr[int (nb) - 1], coef,
                                                 &metrics, &metrics2);
                }
              ...
              if (args->verbose)
                {
                  std::cout << "A Repeat = TRUE flag is sent in order to "
                            ...
                            << std::endl;
                  std::cout << "Main workaround of the new sim logic....."
                            << std::endl;
                }
...
bool
Cells::get_burned (int period, int season, int NMsg, inputs df[],
                 ...
                 bool &activeCrown, int perimeterCells)
{
  ...
  if (ignitionPoints.size () > 0)
  ...
  else
    {
      ...
      df_ptr->ffmc = wdf_ptr->ffmc;
      int head_cell
          = angleToNb[wdf_ptr->waz]; // head cell for slope calculation
      if (head_cell <= 0)            // solve boundaries case
        {
          head_cell
              = this->realId; // as it is used only for slope calculation, if
      ...
      if (args->Simulator == "K")
        ...
        {
          calculate_fbp (&df_ptr[this->realId - 1], coef, &mainstruct,
                         &sndstruct, &headstruct, &flankstruct, &backstruct);
        }
