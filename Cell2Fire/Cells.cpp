 ...
 * landscape).
 */
Cells::Cells(int _id, double _area, std::vector<int> _coord, int _fType,
             std::string _fType2, double _perimeter, int _status, int _realId) {
  ...
  this->id = _id; // identifier for programming purposes: goes from 0 to size of
                  // landscape-1
  this->area = _area;
  this->coord = _coord;
  this->fType = _fType;
  ...
  if (std::abs(4 * this->_ctr2ctrdist - this->perimeter) >
      0.01 * this->perimeter) {
    std::cerr << "Cell ID=" << this->id << "Area=" << this->area
 ...
 * Rate of Spread (ROS) per axis using cell area to compute distances in meters.
 * Initializes the internal dictionaries used for managing fire spread to the
 * cell's neighbors.
 *
 ...
 * @return void
 */
void Cells::initializeFireFields(
    std::vector<std::vector<int>>
        &coordCells, // TODO: should probably make a coordinate type
  ...
  for (auto &nb : adj) {
    ...
    if (nb != -1) {
      ...
      if (availSet.find(nb) != availSet.end()) {
        // TODO: cannot be None, replaced None = -1   and ROSAngleDir has a
        // double inside
 ...
 *
 * Returns the indices of the eight neighboring cells (north, south, east, west,
 * northeast, southeast, southwest, northwest) for the cell in a grid defined by
 * the number of rows and columns. If a neighbor does not exist (e.g., out of
 * bounds), it is marked as -1.
 *
 ...
 * south, northwest, northeast, north}. Missing neighbors are represented by -1.
 */
std::vector<int> adjacentCells(int cell, int nrows, int ncols) {
  ...
  int southeast =
      cell + ncols > total_cells || cell % ncols == 0 ? -1 : cell + ncols + 1;
  int southwest =
      cell % ncols == 1 || cell + ncols > total_cells ? -1 : cell + ncols - 1;
  int northwest = cell % ncols == 1 || cell < ncols ? -1 : cell - ncols - 1;
  std::vector<int> adjacents = {west,  east,      southwest, southeast,
 ...
 * @param back
 */
void Cells::ros_distr_old(double thetafire, double forward, double flank,
                          double back) { // WORKING CHECK OK
...
/**
 * @brief Calculates the radial distance for a given angle in an ellipse defined
 * by its semi-major and semi-minor axes.
 *
 ...
 *
 * @return The radial distance from the ellipse's center to its perimeter at the
 * given angle.
 */
 ...
 * @param EFactor
 */
void Cells::ros_distr(double thetafire, double forward, double flank,
                      double back, double EFactor) { // WORKING CHECK OK
  ...
  std::vector<double> _y = {
      0.0, std::pow(flank, 2) / a, -(std::pow(flank, 2) / a), flank, -flank,
      0.0};
...
/**
 * @brief Distributes the Rate of Spread (ROS) across the cell's neighbors based
 * on fire direction and ellipse geometry.
 *
 * Updates the ROS for each neighbor in the `ROSAngleDir` dictionary using an
 * elliptical model. The ROS is scaled by the elliptical geometry parameters and
 * a factor (`EFactor`), with adjustments based on the fire's heading direction.
 *
 ...
 * @return void
 */
void Cells::ros_distr_V2(double thetafire, double a, double b, double c,
                         double EFactor) {
...
double Cells::allocate(double offset, double base, double ros1,
                       double ros2) { // WORKING CHECK OK
 ...
 * environmental inputs. It determines if the cell begins to spread fire, if so,
 * messages are sent to neighboring cells. It also logs fire metrics for further
 * analysis.
 *
 ...
 * objects.
 * @param args Pointer to a structure containing global simulation arguments and
 * configurations.
 ...
 * @param activeCrown A boolean reference indicating whether crown fire activity
 * is ongoing.
 ...
 * @param CrownFlameLengths A vector tracking the crownfire flame length for
 * each cell.
 * @param CrownIntensities A vector tracking the crown fire intensity for each
 * cell.
 *
...
std::vector<int> Cells::manageFire(
    int period, std::unordered_set<int> &AvailSet, inputs df_ptr[],
    fuel_coefs *coef, std::vector<std::vector<int>> &coordCells,
    std::unordered_map<int, Cells> &Cells_Obj, arguments *args,
    weatherDF *wdf_ptr, std::vector<double> *FSCell,
    std::vector<float> *crownMetrics, bool &activeCrown, double randomROS,
    int perimeterCells, std::vector<int> &crownState,
    std::vector<float> &crownFraction, std::vector<float> &surfFraction,
    std::vector<float> &Intensities, std::vector<float> &RateOfSpreads,
    std::vector<float> &FlameLengths, std::vector<float> &CrownFlameLengths,
  ...
  {
    head_cell = this->realId; // as it is used only for slope calculation, if it
                              // is a boundary cell, it uses the same cell, so
                              // it uses a no slope scenario
  }
  ...
  if (args->Simulator == "K") {
    calculate_k(&df_ptr[this->realId - 1], &df_ptr[head_cell - 1],
                perimeterCells, coef, args, &mainstruct, &sndstruct,
                &headstruct, &flankstruct, &backstruct, activeCrown);
  } else if (args->Simulator == "S") {
    calculate_s(&df_ptr[this->realId - 1], coef, args, &mainstruct, &sndstruct,
                &headstruct, &flankstruct, &backstruct, activeCrown);
  } else if (args->Simulator == "C") {
    calculate_fbp(&df_ptr[this->realId - 1], coef, &mainstruct, &sndstruct,
                  &headstruct, &flankstruct, &backstruct);
  }
  ...
  cartesianAngle =
      360 - (offset >= 360) * (cartesianAngle - 90) - (offset < 360) * offset;
  ...
  if (args->verbose) {
    std::cout << "Main Angle (raz): " << wdf_ptr->waz
              << " Cartesian: " << cartesianAngle << std::endl;
    std::cout << "Front ROS Value: " << headstruct.ros * args->HFactor
              << std::endl;
    std::cout << "Flanks ROS Value: " << flankstruct.ros * args->FFactor
              << std::endl;
    std::cout << "Rear ROS Value: " << backstruct.ros * args->BFactor
              << std::endl;
  ...
  if (args->verbose) {
    ...
    std::cout << "HROS: " << HROS << " Threshold: " << args->ROSThreshold
              << std::endl;
  }
  ...
  if (HROS > args->ROSThreshold) {
    ...
    /*ros_distr(cartesianAngle,
                              ...
                              args->EFactor);
    */
    ros_distr_V2(cartesianAngle, mainstruct.a * args->HFactor,
                 mainstruct.b * args->FFactor, mainstruct.c * args->BFactor,
                 args->EFactor);
    ...
    for (auto &_angle : this->ROSAngleDir) {
      ...
      if (args->verbose) {
        std::cout << "     (angle, realized ros in m/min): (" << angle << ", "
                  << ros << ")" << std::endl;
      }
      if (args->Simulator == "S") {
        // Slope effect
        float se = slope_effect(df_ptr[this->realId - 1].elev,
                                df_ptr[nb - 1].elev, this->perimeter / 4.);
        if (args->verbose) {
          std::cout << "Slope effect: " << se << std::endl;
        }

        // Workaround PeriodLen in 60 minutes
        this->fireProgress[nb] +=
            ros * args->FirePeriodLen * se; // Updates fire progress
      } else
      ...
      if (this->fireProgress[nb] >= this->distToCenter[nb]) {
        ...
        if (args->Simulator == "K") {
          determine_destiny_metrics_k(&df_ptr[int(nb) - 1], coef, args,
                                      &metrics);
        } else if (args->Simulator == "S") {
          determine_destiny_metrics_s(&df_ptr[int(nb) - 1], coef, args,
                                      &metrics);
        } else if (args->Simulator == "C") {
          determine_destiny_metrics_fbp(&df_ptr[int(nb) - 1], coef, &metrics,
                                        &metrics2);
        }
        crownState[this->realId - 1] = mainstruct.crown;
        crownState[nb - 1] = metrics.crown;
        RateOfSpreads[this->realId - 1] =
            double(std::ceil(ros * 100.0) / 100.0);
        ...
        // cannot mutate ROSangleDir during iteration.. we do it like 10 lines
        // down toPop.push_back(angle);
        /*if (verbose) {
      ...
      if (this->fireProgress[nb] < this->distToCenter[nb] && repeat == -100 &&
          -100 != msg_list_aux[0]) {
        if (args->verbose) {
          std::cout << "A Repeat = TRUE flag is sent in order to continue with "
                       "the current fire....."
                    << std::endl;
...
std::vector<int> Cells::manageFireBBO(
    int period, std::unordered_set<int> &AvailSet, inputs *df_ptr,
    fuel_coefs *coef, std::vector<std::vector<int>> &coordCells,
    std::unordered_map<int, Cells> &Cells_Obj, arguments *args,
    weatherDF *wdf_ptr, std::vector<double> *FSCell,
    std::vector<float> *crownMetrics, bool &activeCrown, double randomROS,
    int perimeterCells, std::vector<float> &EllipseFactors,
    std::vector<int> &crownState, std::vector<float> &crownFraction,
  ...
  if (args->verbose) {
    std::cout << "Main Angle (raz): " << wdf_ptr->waz
              << " Cartesian: " << cartesianAngle << std::endl;
    std::cout << "Front ROS Value: " << headstruct.ros * EllipseFactors[0]
              << std::endl;
    std::cout << "Flanks ROS Value: " << flankstruct.ros * EllipseFactors[1]
              << std::endl;
    std::cout << "Rear ROS Value: " << backstruct.ros * EllipseFactors[2]
              << std::endl;
    std::cout << "Std Normal RV for Stochastic ROS CV: " << ROSRV << std::endl;
  }
  ...
  if (HROS > args->ROSThreshold) {
    ...
    /*ros_distr(cartesianAngle,
                              ...
                              EllipseFactors[3]);
    */
    ros_distr_V2(cartesianAngle, mainstruct.a * EllipseFactors[0],
    ...
    for (auto &_angle : this->ROSAngleDir) {
      ...
      // Workaround PeriodLen in 60 minutes
      this->fireProgress[nb] +=
          ros * args->FirePeriodLen; // Updates fire progress
 ...
 * calculations.
 * @param args Pointer to a structure containing global simulation arguments and
 * configurations. The ROS threshold should be stored here with the key
 * "ROSThreshold".
 * @param wdf_ptr Pointer to the weather data structure containing wind speed,
 * direction, and other weather variables.
 * @param activeCrown A boolean reference indicating whether crown fire activity
...
bool Cells::get_burned(int period, int season, int NMsg, inputs df[],
  ...
  //  Calculate parameters
  if (args->Simulator == "K") {
    calculate_k(&(df[this->id]), &(df[head_cell - 1]), perimeterCells, coef,
                args, &mainstruct, &sndstruct, &headstruct, &flankstruct,
                &backstruct, activeCrown);
  } else if (args->Simulator == "S") {
    calculate_s(&(df[this->id]), coef, args, &mainstruct, &sndstruct,
                &headstruct, &flankstruct, &backstruct, activeCrown);
  }

  else if (args->Simulator == "C") {
    calculate_fbp(&df[this->id], coef, &mainstruct, &sndstruct, &headstruct,
                  &flankstruct, &backstruct);
  }
 ...
 */
void Cells::setStatus(int status_int) { this->status = status_int; }
 ...
 */
bool Cells::ignition(int period, int year, std::vector<int> &ignitionPoints,
  ...
  if (ignitionPoints.size() > 0) {
    ...
    if (args->Simulator == "K") {
      ...
      calculate_s(&df_ptr[this->realId - 1], coef, args, &mainstruct,
                  &sndstruct, &headstruct, &flankstruct, &backstruct,
                  activeCrown);
    } else if (args->Simulator == "C") {
      calculate_fbp(&df_ptr[this->realId - 1], coef, &mainstruct, &sndstruct,
                    &headstruct, &flankstruct, &backstruct);
    ...
    if (headstruct.ros * args->HFactor > args->ROSThreshold) {
      if (args->verbose) {
        std::cout << "Head (ROS, FI) values of: ("
                  << headstruct.ros * args->HFactor << ", " << headstruct.fi
...
/*
        ...
        period       int
*/
void Cells::harvested(int id, int period) { // WORKING CHECK OK
  // TODO: unused param
  this->status = 3;
...
/*
        Returns      void
*/
void Cells::print_info() { // WORKING CHECK OK
