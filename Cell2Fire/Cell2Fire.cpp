...
Cell2Fire::Cell2Fire(arguments _args)
  ...
  ********************************************************************/
  ...
  /********************************************************************
                                  Global Values (Forest) and Instance (in memory
  for the moment)
  ********************************************************************/
...
float Cell2Fire::get_co2eq(inputs *df_ptr) {
  ...
  if (this->args.Co2eq) {
    ...
    std::unordered_map<std::string, double> fuel_load = {
        {"C1", 1.575}, {"C2", 5.08}, {"C3", 5.115}, {"C4", 5.12}, {"C5", 5.12},
        {"C6", 5.18},  {"C7", 3.55}, {"D1", 1.5},   {"M1", 5.08}, {"M2", 5.08},
        {"M3", 5.08},  {"M4", 5.08}, {"S1", 8.0},   {"S2", 16.0}, {"S3", 32.0},
        {"O1a", 0.3},  {"O1b", 0.3}};

    // Definir las columnas
    // Fuente Parametros(Gef): 2006 IPCC Guidelines for National Greenhouse Gas
    // Inventories Chapter 2, Vol 4
    std::vector<std::string> fuelTypes = {"O1a", "O1b", "S1", "S2", "S3", "C1",
                                          "C2",  "C3",  "C4", "C5", "C6", "C7",
                                          "D1",  "D2",  "M1", "M2", "M3", "M4"};
    std::vector<int> CO2 = {1613, 1613, 1613, 1613, 1613, 1569,
                            1569, 1569, 1569, 1569, 1569, 1569,
                            1569, 1569, 1569, 1569, 1569, 1569};
    std::vector<double> CH4 = {2.3, 2.3, 2.3, 2.3, 2.3, 4.7, 4.7, 4.7, 4.7,
                               4.7, 4.7, 4.7, 4.7, 4.7, 4.7, 4.7, 4.7, 4.7};
    std::vector<double> N2O = {0.21, 0.21, 0.21, 0.21, 0.21, 0.26,
...
std::unordered_map<int, std::vector<int>> Cell2Fire::SendMessages() {
  ...
  /*
                  ...
                  Burning cells loop: sending messages (Embarrasingly parallel?:
     CP: should be) Each burning cell updates its fire progress and (if needed)
...
void Cell2Fire::Results() {
  ...
  if (this->args.OutFl) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the Flame Lenght to a asc file " << flName
                << std::endl;
    }
...
int main(int argc, char *argv[]) {
  ...
  // std::uniform_int_distribution<int> udistribution(1, args.NWeatherFiles);
  // // Get random weather std::normal_distribution<double>
  // ndistribution(0.0,1.0); // ROSRV
  ...
  // Random ignition
  // std::uniform_int_distribution<int> udistributionIgnition(1, Forest.nCells);
  // // Get random ignition point
...
#pragma omp for
    for (ep = 1; ep <= args.TotalSims; ep++) {
      ...
      // Enforces to satisfy the total number of simulations (if no ignition,
      // find another cell until we obtain the TotalSim asked)
