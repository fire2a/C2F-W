...
__maintainer__ = "Jaime Carrasco, Cristobal Pais, David Woodruff, David
Palacios"
*/
...
inputs *df_ptr;
weatherDF *wdf_ptr;
weatherDF wdf[100000];
// hard to dynamic allocate memory since it changes from file to file, better to
// keep constant size;
inputs *df;
int currentSim = 0;
 ...
 *
 */
void printSets(std::unordered_set<int> availCells,
...
// Output grids
void CSVGrid(int rows, int cols, int gridNumber, std::string gridFolder,
             std::vector<int> &statusCellsCSV, bool verbose) {
  std::string gridName;
  if (gridNumber < 10) {
  ...
  if (verbose) {
    std::cout << "We are plotting the current forest to a csv file " << gridName
              << std::endl;
  }
 ...
 * and data files required for simulating fire spread in a forested environment.
 * It populates various member variables, including fuel coefficients, forest
 * structure, weather data, ignition points, and other simulation-related
 * parameters. It also performs preprocessing steps to categorize cells into
 * burnable and non-burnable types and to establish fire ignition sets.
 *
 * @param _args List of command line arguments containing configuration and file
 * paths needed for the simulation.
 *
...
Cell2Fire::Cell2Fire(arguments _args)
    : CSVWeather(_args.InFolder + "Weather.csv", ","),
  ...
  ********************************************************************/
  ...
  // iterates from the first element of df, using DF, args_ptr and the number of
  // cells
  ...
  std::cout << "\nWeather DataFrame from instance " << this->CSVWeather.fileName
            << std::endl;
  ...
  this->CSVWeather.parseWeatherDF(wdf_ptr, this->args_ptr, this->WeatherDF,
                                  WPeriods);
  // std::cout << "\nPass Debug " << std::endl;
  ...
  if (this->args.Ignitions) {
    ...
    CSVIgnitions.parseIgnitionDF(this->IgnitionPoints, IgnitionsDF,
                                 IgnitionYears);
    // this->IgnitionSets =
    ...
    if (this->args.IgnitionRadius > 0) {
      ...
      for (i = 0; i < this->args.TotalYears; i++) {
        ...
        std::vector<int> adjacentCellsVector =
            adjacentCells(igVal, this->rows, this->cols);
        ...
        if (this->args.IgnitionRadius > 1) {
          ...
          while (auxR < args.IgnitionRadius) {
            ...
            for (auto &c : IgnitionSetsAux) {
              // Populate Aux Set
              std::vector<int> adjacentCellsVector =
                  adjacentCells(c, this->rows, this->cols);
  ...
  if (this->args.WeatherOpt.compare("rows") == 0 ||
      this->args.WeatherOpt.compare("random") == 0) {
    ...
    if (this->args.MaxFirePeriods > maxFP) {
      ...
      if (this->args.verbose) {
        std::cout << "Maximum fire periods are set to: "
                  ...
                  << " based on the weather file, Fire Period Length, and "
                     "Minutes per WP"
                  << std::endl;
      }
    }
 ...
 * map. It also initializes the fire-related fields for the cell and, if verbose
 * mode is enabled, prints debugging information about the cell.
 *
...
void Cell2Fire::InitCell(int id) {
  ...
  Cells Cell(id - 1, this->areaCells, this->coordCells[id - 1],
             this->fTypeCells[id - 1], this->fTypeCells2[id - 1],
...
void Cell2Fire::reset(int rnumber, double rnumber2, int simExt = 1) {
  ...
  */
  ...
  if (this->args.WeatherOpt.compare("random") == 0) {
    ...
    std::cout << "\nWeather file selected: " << this->CSVWeather.fileName
              << std::endl;
    ...
    // Check maxFirePeriods and Weather File consistency (reset MaxFirePeriods
    // and recalculate)
    int maxFP = this->args.MinutesPerWP / this->args.FirePeriodLen * WPeriods;
    ...
    // this->args.FirePeriodLen << std::endl; DEBUGstd::cout << "MaxfirePeriods:
    // " << this->args.MaxFirePeriods << std::endl;
    if (this->args.MaxFirePeriods > maxFP) {
      this->args.MaxFirePeriods = maxFP;
      if (this->args.verbose) {
        std::cout << "Maximum fire periods are set to: "
  ...
  if (this->args.WeatherOpt.compare("distribution") == 0) {
    ...
    std::string weather_name;
    while (!selectWeather) {
      std::default_random_engine generator3(args.seed * simExt * time(NULL));
      // creates a different generator solving cases when parallel running
      // creates simulations at same time
      ...
      if (probability_weather > rd_number) {
        // rd_number cannot be 1, but can be 0. So if prob weather has prob=0 it
        // does not have a chance of being selected, and if it is 1, it will
        // always be selected
    ...
    this->CSVWeather.fileName =
        this->args.InFolder + "Weathers" + separator() + weather_name + ".csv";
    WeatherHistory.push_back(weather_name);
 ...
 * from predefined data (e.g., CSV files), and attempting to ignite those points
 * based on the conditions of the cells (availability, burnability, etc.). The
 * simulation state is updated accordingly, including tracking ignited, burnt,
 * and available cells, as well as the fire and weather periods.
 *
 ...
 *
 * ### Detailed Behavior:
 * - If no ignition points are provided (args.Ignitions == 0), a random point is
 * selected repeatedly until a valid ignition is found or a limit is reached.
 ...
 * - The ignition process updates the state of the forest, including burning and
 * burnt cells, and prepares for the next period or year if no ignition occurs.
 *
 * ### Key Updates:
 ...
 *
 * ### Verbose Mode:
 * When `args.verbose` is true, detailed logs of the ignition process, including
 * selected points, random values, and status updates, are printed to the
 * console.
 */
...
bool Cell2Fire::RunIgnition(std::default_random_engine generator, int ep) {
  ...
  std::default_random_engine generator2(args.seed * ep * this->nCells);
  // * time(NULL)); //creates a different generator solving cases when parallel
  ...
  if (this->args.Ignitions == 0) {
    while (true) {
      ...
      while (true) {
        ...
        // DEBUGstd::cout << "selectedProbability: " << ignProb[selected-1] <<
        // std::endl; DEBUGstd::cout << "randomNumber: " << rd_number <<
        // std::endl; if (ignProb[selected - 1] / static_cast<float>(100) >
        // rd_number) {    //If probabilities enter as integers, e.g: 80 as 0.8
      ...
      if (this->statusCells[aux - 1] < 3 &&
          this->burntCells.find(aux) == this->burntCells.end()) {
        ...
        if (it->second.getStatus() == "Available" && it->second.fType != 0) {
          ...
          std::cout << "\nSelected (Random) ignition point for Year "
                    << this->year << ", sim " << this->sim << ": " << aux;
          std::vector<int> ignPts = {aux};
          if (it->second.ignition(this->fire_period[year - 1], this->year,
                                  ignPts, &df[aux - 1], this->coef_ptr,
                                  this->args_ptr, &wdf[this->weatherPeriod],
                                  this->activeCrown, this->perimeterCells)) {
            // Printing info about ignitions
            if (this->args.verbose) {
              std::cout << "Cell " << it->second.realId << " Ignites"
                        << std::endl;
              std::cout << "Cell " << it->second.realId
  ...
  // Ignitions with provided points from CSV
    ...
    if (this->args.IgnitionRadius > 0) {
      ...
      std::uniform_int_distribution<int> udistribution(
          0, this->IgnitionSets[this->year - 1].size() - 1);
      temp = this->IgnitionSets[this->year - 1][udistribution(generator)];
    }

    std::cout << "\nSelected ignition point for Year " << this->year << ", sim "
              << this->sim << ": " << temp;
    // this->
    IgnitionHistory[sim] = temp;

    // If cell is available
    if (this->burntCells.find(temp) == this->burntCells.end() &&
        this->statusCells[temp - 1] < 3) {
      ...
      if (it->second.getStatus() == "Available" && it->second.fType != 0) {
        ...
        if (it->second.ignition(this->fire_period[year - 1], this->year, ignPts,
                                &df[temp - 1], this->coef_ptr, this->args_ptr,
                                &wdf[this->weatherPeriod], this->activeCrown,
                                this->perimeterCells)) {
          // Printing info about ignitions
          if (this->args.verbose) {
            std::cout << "Cell " << it->second.realId << " Ignites"
      ...
      if (this->args.verbose) {
        std::cout << "No ignition during year " << this->year << ", Cell "
                  << this->IgnitionPoints[this->year - 1]
                  << " is already burnt or non-burnable type" << std::endl;
      }
      this->year++;
      this->weatherPeriod = 0;
    }
  ...
  if (this->args.verbose) {
    std::cout << "Fire period updated to " << this->fire_period[year - 1]
              << std::endl;
  }
  ...
  if (this->args.verbose) {
    std::cout << "Fire Period Starts: " << this->fire_period[year - 1]
              << std::endl;
    ...
    std::cout << "\nNext Fire Period: " << this->fire_period[year - 1]
              << std::endl;
  }
 ...
 *
 * @return A map of cell IDs to lists of neighboring cell IDs that received fire
 * messages. Each key is the ID of a burning cell, and the value is a list of
 * neighboring cell IDs affected by the fire.
 *
 * ### Detailed Behavior:
 ...
 * - Prints detailed logs of the fire progress and messages sent by each burning
 * cell.
...
std::unordered_map<int, std::vector<int>> Cell2Fire::SendMessages() {
  ...
  if (fire_period[year - 1] == args.MaxFirePeriods - 1 && this->args.verbose) {
    std::cout << "*** WARNING!!! About to hit MaxFirePeriods: "
              << this->args.MaxFirePeriods << std::endl;
  }
  ...
  if (this->args.verbose) {
    std::cout << "\n---------------------- Step 2: Sending Messages from "
                 "Ignition ----------------------"
              << std::endl;
    std::cout << "Current Fire Period:" << this->fire_period[this->year - 1]
              << std::endl;
    printSets(this->availCells, this->nonBurnableCells, this->burningCells,
              this->burntCells, this->harvestCells);
  }
  ...
  /*
          ...
          Burning cells loop: sending messages (Embarrasingly parallel?: CP:
     should be) Each burning cell updates its fire progress and (if needed)
     populates their message
  */
    ...
    if (it->second.ROSAngleDir.size() > 0) {
      ...
      if (!this->args.BBOTuning) {
        ...
        aux_list = it->second.manageFire(
            this->fire_period[this->year - 1], this->availCells, df,
            this->coef_ptr, this->coordCells, this->Cells_Obj, this->args_ptr,
            &wdf[this->weatherPeriod], &this->FSCell, &this->crownMetrics,
            this->activeCrown, this->ROSRV, this->perimeterCells,
            this->crownState, this->crownFraction, this->surfFraction,
            this->Intensities, this->RateOfSpreads, this->FlameLengths,
            this->CrownFlameLengths, this->CrownIntensities);
      } else {
        // TODO  MAY 11: Pass the slope factor or the slopes of the adjacent
        // cells
        auto factors = BBOFactors.find(NFTypesCells[cell - 1]);
        aux_list = it->second.manageFireBBO(
            this->fire_period[this->year - 1], this->availCells, &df[cell - 1],
            this->coef_ptr, this->coordCells, this->Cells_Obj, this->args_ptr,
            &wdf[this->weatherPeriod], &this->FSCell, &this->crownMetrics,
            this->activeCrown, this->ROSRV, this->perimeterCells,
            factors->second, this->crownState, this->crownFraction,
            this->surfFraction, this->Intensities, this->RateOfSpreads,
            this->FlameLengths);
      }
      ...
      if (this->args.verbose)
        std::cout
            << "\nCell " << cell
            << " does not have any neighbor available for receiving messages"
            << std::endl;
    }
 ...
 * cells. Based on the received messages, cells may ignite, continue burning, or
 * be marked as burnt out. The method also manages the transitions between fire
 * periods and updates relevant cell sets (e.g., available, burning, burnt).
 *
 ...
 * fire model.
 */
void Cell2Fire::GetMessages(
    std::unordered_map<int, std::vector<int>> sendMessageList) {
  // Iterator
  std::unordered_map<int, Cells>::iterator it;
  ...
  if (this->repeatFire && this->messagesSent) {
    if (this->args.verbose)
      std::cout << "Messages have been sent, next step. Current period: "
                << this->fire_period[this->year - 1] << std::endl;
    this->repeatFire = false;
  }
  ...
  if (!this->messagesSent && !this->repeatFire) {
    if (this->args.verbose) {
      std::cout << "\nNo messages during the fire period, end of year "
                << this->year << std::endl;
    }
    ...
    for (auto &_bc : globalMessagesList) {
      ...
      if (this->burntCells.find(bc) == this->burntCells.end()) {
        ...
        if (it->second.fType != 0) {
          checkBurnt = it->second.get_burned(
              this->fire_period[this->year - 1], 1, this->year, df,
        ...
        if (this->args.verbose) {
          std::cout << "\nCell " << it->second.realId
                    << " got burnt (1 true, 0 false): " << checkBurnt
                    << std::endl;
        }
...
void Cell2Fire::Results() {
  ...
  std::cout
      << "\n----------------------------- Results -----------------------------"
      << std::endl;
  std::cout << "Total Available Cells:    " << ACells
            << " - % of the Forest: " << ACells / nCells * 100.0 << "%"
            << std::endl;
  std::cout << "Total Burnt Cells:        " << BCells
            << " - % of the Forest: " << BCells / nCells * 100.0 << "%"
            << std::endl;
  std::cout << "Total Non-Burnable Cells: " << NBCells
            << " - % of the Forest: " << NBCells / nCells * 100.0 << "%"
            << std::endl;
  std::cout << "Total Firebreak Cells: " << HCells
            << " - % of the Forest: " << HCells / nCells * 100.0 << "%"
            << std::endl;
  ...
  if (this->args.FinalGrid) {
    ...
    if (this->args.OutFolder.empty())
      this->gridFolder = this->args.InFolder + "simOuts" + separator() +
                         "Grids" + separator() + "Grids" +
                         std::to_string(this->sim) + separator();
    else
      this->gridFolder = this->args.OutFolder + "Grids" + separator() +
                         "Grids" + std::to_string(this->sim) + separator();
  ...
  if (this->args.OutMessages) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the network messages to a csv file "
                << messagesName << std::endl;
    }
  ...
  if (this->args.OutRos) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the Rate of Spread to a asc file "
                << rosName << std::endl;
    }
    CSVWriter CSVPloter(rosName, " ");
    CSVPloter.printASCII(this->rows, this->cols, this->xllcorner,
                         this->yllcorner, this->cellSide, this->RateOfSpreads);
  }
  ...
  if ((this->args.OutCrownConsumption) && (this->args.AllowCROS)) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the Crown Fraction Burn to a asc file "
                << cfbName << std::endl;
    }
    CSVWriter CSVPloter(cfbName, " ");
    CSVPloter.printASCII(this->rows, this->cols, this->xllcorner,
                         this->yllcorner, this->cellSide, this->crownFraction);
  }
  ...
  if ((this->args.OutSurfConsumption) && (this->args.Simulator == "C")) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the Surface Fraction Burn to a asc file "
                << sfbName << std::endl;
    }
    CSVWriter CSVPloter(sfbName, " ");
    CSVPloter.printASCII(this->rows, this->cols, this->xllcorner,
                         this->yllcorner, this->cellSide, this->surfFraction);
  }
  ...
  if ((this->args.OutCrown) && (this->args.AllowCROS)) {
    ...
    if (this->args.verbose) {
      std::cout << "We are generating the Crown behavior to a asc file "
                << crownName << std::endl;
    }
    CSVWriter CSVPloter(crownName, " ");
    // CSVPloter.printCrownAscii(this->rows, this->cols, this->xllcorner,
    // this->yllcorner, this->cellSide, this->crownMetrics, statusCells2); /OLD
    // VERSION
    CSVPloter.printASCIIInt(this->rows, this->cols, this->xllcorner,
                            this->yllcorner, this->cellSide, this->crownState);
  }
  ...
  if (currentSim == args.TotalSims && this->args.IgnitionsLog) {
    ...
    this->ignitionsFolder =
        this->args.OutFolder + "IgnitionsHistory" + separator();
 ...
 * the simulation. The file is saved with a unique name in the designated output
 * folder.
 *
 * ### Behavior:
 * - **Cell Status Update**:
 ...
 * - **Verbose Mode**:
 *   - Logs the file name and output process to the console if `args.verbose` is
 * enabled.
 * - **Output Format**:
 ...
 * specified criteria.
 *
 * This method dynamically updates the weather parameters used in the simulation
 * based on the simulation's fire period and weather options. It can optionally
 * output the current state of the forest grid and log weather updates if
...
void Cell2Fire::updateWeather() {
  if (this->args.WeatherOpt != "constant" && this->fire_period[this->year - 1] *
                                                     this->args.FirePeriodLen /
                                                     this->args.MinutesPerWP >
 ...
 *
 * This method simulates one fire period, handling all related processes such as
 * ignition, message exchange, weather updates, and transitioning between
 * simulation years or scenarios. It is the core operational logic for advancing
 * the simulation in discrete steps.
 *
 * ### Behavior:
 * - **Verbose Logging**:
 ...
 * termination are met.
 *   - Handles additional simulation-specific logic based on weather options and
...
void Cell2Fire::Step(std::default_random_engine generator, int ep) {
  ...
  if (this->args.verbose) {
    ...
    std::cout << "Fire Period: " << this->fire_period[this->year - 1]
              << std::endl;
  ...
  // Ending conditions
  if (this->fire_period[std::min(this->year, int(fire_period.size())) - 1] >=
      this->args.MaxFirePeriods) {
  ...
  if ((this->availCells.size() == 0) ||
      (this->burningCells.size() == 0 && !this->noMessages) && !this->done &&
          !auxC) {
    // Done
    this->done = true;
    // Print-out results to folder
  ...
  if ((this->sim > args.TotalSims) && (args.WeatherOpt != "rows")) {
    ...
    if (this->counter_wt <= 1) {
      ...
      this->historyFolder =
          this->args.OutFolder + "WeatherHistory" + separator();
...
void Cell2Fire::InitHarvested() { std::cout << "OK"; }
 ...
 * - Executing simulation episodes in parallel, with each episode representing a
 * replication.
...
#pragma omp parallel num_threads(num_threads)
  {
    ...
    std::default_random_engine &generator =
        generators[TID]; // generators[args.nthreads]
    // Random generator and distributions
    std::uniform_int_distribution<int> udistribution(
        1, args.NWeatherFiles); // Get random weather
    std::normal_distribution<double> ndistribution(0.0, 1.0); // ROSRV
    // Random numbers (weather file and ROS-CV)
    int rnumber;
    double rnumber2;
    std::uniform_int_distribution<int> udistributionIgnition(
        1, Forest.nCells); // Get random ignition point
    // Steps and stop
    int ep = 0;
    int tstep = 0;
    int stop = 0;
#pragma omp for
    for (ep = 1; ep <= args.TotalSims; ep++) {
      ...
      for (tstep = 0;
           tstep <= Forest.args.MaxFirePeriods * Forest.args.TotalYears;
           tstep++) {
