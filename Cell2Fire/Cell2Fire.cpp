...
// Output grids
void
CSVGrid (int rows, int cols, int gridNumber, std::string gridFolder,
         std::vector<int> &statusCellsCSV, bool verbose)
{
  ...
  if (gridNumber < 10)
    {
      gridName
          = gridFolder + "ForestGrid0" + std::to_string (gridNumber) + ".csv";
    }

  else
    {
      gridName
          = gridFolder + "ForestGrid" + std::to_string (gridNumber) + ".csv";
    }

  if (verbose)
    {
      std::cout << "We are plotting the current forest to a csv file "
                << gridName << std::endl;
    }
      ...
      CSVForest (_args.InFolder + "fuels", " ")
{
  ...
  ********************************************************************/
  ...
  if (strcmp (this->args.HarvestPlan.c_str (), EM) != 0)
    {
      ...
      for (auto it = HarvestedCells.begin (); it != HarvestedCells.end ();
           it++)
        {
          std::cout << " " << it->first << ": ";
  ...
  std::cout << "\nWeather DataFrame from instance "
            << this->CSVWeather.fileName << std::endl;

  if (this->args.WeatherOpt.compare ("distribution") == 0)
    {
      CSVReader CSVWeatherDistribution (
          _args.InFolder + "WeatherDistribution.csv", ",");
  ...
  if (this->args.Ignitions)
    {
      ...
      // Populate Ignitions vector
      std::vector<std::vector<std::string> > IgnitionsDF
          = CSVIgnitions.getData ();
      ...
      this->IgnitionSets
          = std::vector<std::vector<int> > (this->args.TotalYears);
                  ...
                  while (auxR < args.IgnitionRadius)
                    {
                      ...
                      std::vector<int> IgnitionSetsAux
                          = this->IgnitionSets[auxIg];
  ...
  if (this->args.WeatherOpt.compare ("rows") == 0
    ...
    {
      int maxFP
          = this->args.MinutesPerWP / this->args.FirePeriodLen * WPeriods;
...
Cell2Fire::reset (int rnumber, double rnumber2, int simExt = 1)
{
  ...
  if (this->args.OutIntensity)
    {
      ...
      this->intensityFolder
          = this->args.OutFolder + "Intensity" + separator ();
    }
  // Flame Length Folder
  if (this->args.OutFl)
    {
  ...
  if (this->args.OutCrownConsumption && this->args.AllowCROS)
    {
      ...
      this->cfbFolder = this->args.OutFolder + separator ()
                        + "CrownFractionBurn" + separator ();
    }
  ...
  if (this->args.OutSurfConsumption && this->args.Simulator == "C")
    {
      ...
      this->sfbFolder = this->args.OutFolder + separator ()
                        + "SurfFractionBurn" + separator ();
    }
  ...
  */
  ...
  if (this->args.WeatherOpt.compare ("random") == 0)
    {
      // Random Weather
      this->CSVWeather.fileName = this->args.InFolder + "Weathers"
                                  + separator () + "Weather"
                                  + std::to_string (rnumber) + ".csv";
      ...
      // Populate the wdf objects
      this->CSVWeather.parseWeatherDF (wdf_ptr, this->args_ptr,
                                       this->WeatherDF, WPeriods);
      // DEBUGthis->CSVWeather.printData(this->WeatherDF);
      ...
      while (!selectWeather)
        {
          std::default_random_engine generator3 (
              args.seed * simExt
              * time (
                  NULL)); // creates a different generator solving cases when
...
Cell2Fire::RunIgnition (std::default_random_engine generator, int ep)
{
  ...
  std::default_random_engine generator2 (
      args.seed * ep
      * this->nCells); // * time(NULL)); //creates a different generator
                       // solving cases when parallel running creates
          ...
          while (true)
            {
              ...
              float rd_number
                  = (float)rand () / ((float)(RAND_MAX / 0.999999999));
              ...
              if (it->second.getStatus () == "Available"
                  && it->second.fType != 0)
                {
                  ...
                  std::cout << "\nSelected (Random) ignition point for Year "
                            << this->year << ", sim " << this->sim << ": "
                            << aux;
                  std::vector<int> ignPts = { aux };
                  if (it->second.ignition (
                          this->fire_period[year - 1], this->year, ignPts,
                          &df[aux - 1], this->coef_ptr, this->args_ptr,
  ...
  // Ignitions with provided points from CSV
  else
    {
      ...
      std::cout << "\nSelected ignition point for Year " << this->year
                << ", sim " << this->sim << ": " << temp;
      // this->
      ...
      if (this->burntCells.find (temp) == this->burntCells.end ()
      ...
      else
        {
          ...
          if (this->args.verbose)
            {
              std::cout << "No ignition during year " << this->year
                        << ", Cell " << this->IgnitionPoints[this->year - 1]
                        << " is already burnt or non-burnable type"
                        << std::endl;
            }
          this->year++;
          this->weatherPeriod = 0;
  ...
  if (!this->noIgnition)
    {
      ...
      if (this->args.verbose)
        {
          printSets (this->availCells, this->nonBurnableCells,
                     this->burningCells, this->burntCells, this->harvestCells);
        }
    }
          ...
          if (!this->args.BBOTuning)
            { //&df[cell-1] replaced by full df for getting the slopes
              aux_list = it->second.manageFire (
                  ...
                  this->coef_ptr, this->coordCells, this->Cells_Obj,
                  this->args_ptr, &wdf[this->weatherPeriod], &this->FSCell,
                  &this->crownMetrics, this->activeCrown, this->ROSRV,
                  this->perimeterCells, this->crownState, this->crownFraction,
          ...
          else
            {
              ...
              aux_list = it->second.manageFireBBO (
                  this->fire_period[this->year - 1], this->availCells,
                  &df[cell - 1], this->coef_ptr, this->coordCells,
                  this->Cells_Obj, this->args_ptr, &wdf[this->weatherPeriod],
                  &this->FSCell, &this->crownMetrics, this->activeCrown,
                  this->ROSRV, this->perimeterCells, factors->second,
  ...
  if (this->repeatFire && !this->messagesSent)
    {
      if (this->args.verbose)
        {
          std::cout << "Fires are still alive, no message generated"
                    << std::endl;
          std::cout << "Current fire period: "
...
Cell2Fire::Results ()
{
  ...
  if (this->args.OutMessages)
    {
      ...
      messagesName
          = this->messagesFolder + "MessagesFile" + oss.str () + ".csv";
      if (this->args.verbose)
        {
  ...
  if (this->args.OutIntensity)
    {
      ...
      oss << std::setfill ('0') << std::setw (this->widthSims) << this->sim;
      intensityName
          = this->intensityFolder + "Intensity" + oss.str () + ".asc";
  ...
  if (this->args.OutFl)
    {
      ...
      if (this->args.verbose)
        {
          std::cout << "We are generating the Flame Lenght to a asc file "
                    << flName << std::endl;
        }
  ...
  if ((this->args.OutCrownConsumption) && (this->args.AllowCROS))
    {
      this->cfbFolder
          = this->args.OutFolder + "CrownFractionBurn" + separator ();
      std::string cfbName;
      std::ostringstream oss;
      oss.str ("");
      oss << std::setfill ('0') << std::setw (this->widthSims) << this->sim;
      ...
      if (this->args.verbose)
        {
          std::cout
              << "We are generating the Crown Fraction Burn to a asc file "
              << cfbName << std::endl;
        }
  ...
  if ((this->args.OutSurfConsumption) && (this->args.Simulator == "C"))
    {
      this->sfbFolder
          = this->args.OutFolder + "SurfFractionBurn" + separator ();
      std::string sfbName;
      std::ostringstream oss;
      oss.str ("");
      oss << std::setfill ('0') << std::setw (this->widthSims) << this->sim;
      ...
      if (this->args.verbose)
        {
          std::cout
              << "We are generating the Surface Fraction Burn to a asc file "
              << sfbName << std::endl;
        }
...
Cell2Fire::updateWeather ()
{
  if (this->args.WeatherOpt != "constant"
      && this->fire_period[this->year - 1] * this->args.FirePeriodLen
  ...
  if (this->args.verbose)
    {
      ...
      std::cout << "MaxFirePeriods: " << this->args.MaxFirePeriods
                << std::endl;
      printSets (this->availCells, this->nonBurnableCells, this->burningCells,
                 this->burntCells, this->harvestCells);
      std::cout << "********************************************" << std::endl;
