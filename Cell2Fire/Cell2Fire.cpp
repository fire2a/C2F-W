...
Cell2Fire::Cell2Fire(arguments _args)
  ...
  ********************************************************************/
  ...
  /********************************************************************
                  Global Values (Forest) and Instance (in memory for the
  moment)
  ********************************************************************/
  ...
  ********************************************************************/
  ...
  CSVParser.parseDF(df_ptr, DF, this->args_ptr,
                    this->nCells); // iterates from the first element of df,
                                   // using DF, args_ptr and the number of cells
  ...
  */
  ...
  if (this->args.WeatherOpt.compare("distribution") == 0) {
    ...
    while (!selectWeather) {
      ...
      if (probability_weather >
          rd_number) { // rd_number cannot be 1, but can be 0. So if prob
                       // weather has prob=0 it does not have a chance of being
                       // selected, and if it is 1, it will always be selected
    ...
    // DEBUGstd::cout << "MinutesPerWP: " << this->args.MinutesPerWP <<
    // std::endl; DEBUGstd::cout << "FirePeriodLen: " <<
    // this->args.FirePeriodLen << std::endl;
    // DEBUGstd::cout << "MaxfirePeriods: " << this->args.MaxFirePeriods <<
    // std::endl;
...
bool Cell2Fire::RunIgnition(std::default_random_engine generator, int ep) {
  ...
  std::default_random_engine generator2(
      args.seed * ep *
      this->nCells); // * time(NULL)); //creates a different generator solving
                     // cases when parallel running creates simulations at same
                     // time
  std::unordered_map<int, Cells>::iterator it;
  ...
  if (this->args.verbose) {
    ...
    if (this->args.WeatherOpt.compare("constant") == 0)
      std::cout << "(NOTE: current weather is not used for ROS with constant "
                   "option)"
                << std::endl;
  ...
  for (int cell : this->burningCells) {
    ...
    */
    if (it->second.ROSAngleDir.size() > 0) {
      ...
      if (!this->args.BBOTuning) { //&df[cell-1] replaced by full df for getting
                                   // the slopes
