...
float rate_of_spread_k(
    inputs *data, fuel_coefs *ptr,
    main_outs *at) // incluir efecto pendiente aqui y no afuera
{
  ...
  fch =
      (389.1624 + 14.3 * ch + 0.02 * pow(ch, 2.0)) /
...
float slope_effect(float elev_i, float elev_j, int cellsize) {
  float ps_ij =
      (elev_j - elev_i) /
      (cellsize / 4.); // cellsize corresponds to the perimeter of the cell
  float se;
...
{
  float p1, p2, p3, ws, tmp, rh, ch, fmc, fch, fv, ps, ros_active, rospl04, fp,
      ros_final, ros;
 ...
 */
void calculate_k(inputs *data, inputs *head, int cellsize, fuel_coefs *ptr,
                 arguments *args, main_outs *at, snd_outs *sec,
  ...
  if (args->AllowCROS && cbhs[data->nftype][0] != 0) {
    if (activeCrown) { // si el fuego esta activo en copas chequeamos
                       // condiciones
...
void determine_destiny_metrics_k(inputs *data, fuel_coefs *ptr, arguments *args,
                                 main_outs *metrics) {
  // Hack: Initialize coefficients
  setup_const();
