                  ...
                  main_outs *at) // incluir efecto pendiente aqui y no afuera
{
  ...
  fch = (389.1624 + 14.3 * ch + 0.02 * pow (ch, 2.0))
        / (3.559 + 1.6615 * ch
           + 2.62392 * pow (ch, 2.0)); // es -14.3 segun el libro
  fv = p1 * exp (-p2 * ws) + p3;
  if (ps == 0)
    {
      at->rss = fmc * fch * (fv);
 ...
 */
void
calculate_k (inputs *data, inputs *head, int cellsize, fuel_coefs *ptr,
             arguments *args, main_outs *at, snd_outs *sec, fire_struc *hptr,
...
void
determine_destiny_metrics_k (inputs *data, fuel_coefs *ptr, arguments *args,
                             main_outs *metrics)
{
  // Hack: Initialize coefficients
  setup_const ();
